...
function useAuth(supabaseClient: any) {
  ...
  const signUp = useCallback(async (email: string, password: string) => {
    ...
    try {
      ...
      if (signInError) {
        console.warn('Auto sign-in failed, user needs to verify email:', signInError)
        return null
      }
...
export default function StartConsultationPage() {
    ...
    const initializePage = async () => {
      try {
        ...
        if (userProfile) {
          ...
          if (subscription && userProfile.profile_completed) {
  ...
  const handleAuth = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authView === "login") {
        ...
        if (profile?.profile_completed && patient) {
  ...
  }, [authView, loginForm, signupForm, signIn, signUp, router, setAuthError, supabaseClient])
    ...
    switch (currentStep) {
      ...
      case 3:
        ...
        try {
          ...
          if (existingPatient) {
            ...
            const { data: updatedPatient, error: updateError } = await supabaseClient
              ...
              .update({
                ...
                is_local: patientForm.country?.toLowerCase() === 'mauritius' || patientForm.country?.toLowerCase() === 'maurice',
          ...
          if (existingSub) {
            ...
            const { error: subError } = await supabaseClient
              ...
              .insert({
                ...
                consultations_limit: subscriptionType === "solo" ? 1 : null,
                patient_first_name: patientForm.firstName,
                patient_last_name: patientForm.lastName,
                patient_email: user.email,
              })
          ...
          // If pay per use, go directly to dashboard
          if (selectedPlan !== "solo") {
            router.push("/dashboard")
            return
          }
          
          // For solo plan, go to payment
          setCurrentStep(4)
        } catch (error: any) {
  ...
  }, [currentStep, user, selectedPlan, patientForm, supabaseClient, router])
